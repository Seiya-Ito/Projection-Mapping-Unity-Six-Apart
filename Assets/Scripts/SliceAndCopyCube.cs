...
public class SliceAndCopyCube : MonoBehaviour {
	
	private const int NUM = 320;
	
	public List<IEvent> eventList = new List<IEvent> ();
	
	public MyObject[] objs = new MyObject[NUM*2];
	private int scene = 0;
	...
	void Start () {
		...
		for (int i=0; i<NUM; ++i) {
			objs[i+NUM] = new MyObject();
			objs[i+NUM].obj = Instantiate(Resources.Load("Prefabs/boxes"), new Vector3(0,-0.1625f+0.001f*i,0), Quaternion.Euler(90, 45, 0)) as GameObject;
		}
	...
	class RotationEvent : IEvent {
		...
		public SliceAndCopyCube scene;
		public MyObject[] objs;
		
		private int[] frame;
		
		public RotationEvent(SliceAndCopyCube scene) {
			this.scene = scene;
	...
	class MovingEvent : IEvent {
		...
		Vector3 dir2 = new Vector3 (-0.5f, -0.5f, 0);

		bool flag = false;
		
		public MovingEvent(SliceAndCopyCube scene, float distance = 0.32f) {
			this.scene = scene;
			this.objs = scene.objs;
		...
		public void action() {
			...
			for (int i=NUM; i<NUM*2; ++i) {
				if (objs [i].frame++ > time) {
					...
					objs[i].obj.transform.Translate(dir2 * delta);
					if (objs [i].frame < wait)
						break;
				}
			}
			if (objs [NUM*2 - 1].frame > time+1) {
	...
	class ScaleEvent : IEvent {
		...
		public ScaleEvent(SliceAndCopyCube scene) {
			this.scene = scene;
			this.objs = scene.objs;
