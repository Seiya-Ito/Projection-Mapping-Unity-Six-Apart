...
public class WaveCube : MonoBehaviour {
	...
	class RotationEvent : IEvent {
		...
		public WaveCube scene;
		public MyObject[] objs;
		
		private int[] frame;
		
		public RotationEvent(WaveCube scene) {
			this.scene = scene;
	...
	class MovingEvent : IEvent {
		...
		public MovingEvent(WaveCube scene, float distance = 0.32f) {
			this.scene = scene;
			this.objs = scene.objs;
	...
	class ScaleEvent : IEvent {
		...
		public ScaleEvent(WaveCube scene) {
			this.scene = scene;
			this.objs = scene.objs;
		...
		public void action() {
			...
			const int time = 60;
			for (int i=0; i<NUM; ++i) {
				float scale = (Mathf.Cos (-pos / 5.0f + i / 20.0f) + 1.0f) / 10.0f + 0.8f;
